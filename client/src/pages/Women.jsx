  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Filter products for women's category - Allow 'women', 'woman', 'female' case-insensitive
        const womenProducts = res.data.products.filter(product => {
          return product.category && ['women', 'woman', 'female'].includes(product.category.trim().toLowerCase());
        });
    ...
    <div className="min-h-screen bg-gray-950 text-white font-sans transition-all duration-500 overflow-hidden">
      ...
      <section className="py-24 px-6 text-center relative overflow-hidden bg-gray-950">
        ...
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-wide drop-shadow-xl">
          <span className="bg-gradient-to-r from-fuchsia-400 to-indigo-500 text-transparent bg-clip-text">Women's</span> Collection
        </h1>
        <p className="mt-6 text-xl text-gray-300 max-w-2xl mx-auto">
          Discover stunning fashion pieces designed for the modern woman.
        </p>
      </section>
      ...
      <section className="py-20 px-6 bg-gray-900 max-w-screen-xl mx-auto rounded-xl">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {products.map((product) => (
              <Link to={`/product/${product._id}`} key={product._id} className="block group">
                <div className="bg-gradient-to-br from-[#f3e8ff] to-[#e0e7ff] dark:from-[#1a1a2e] dark:to-[#16213e] rounded-2xl shadow-2xl p-4 hover:scale-105 transition">
          ...
          <div className="text-center py-20">
            <h3 className="text-2xl font-semibold mb-4">No Women's Products Found</h3>
      ...
      <section className="bg-gray-900 text-white py-20 text-center mt-20">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <Link to="/category/kids" className="bg-gradient-to-r from-white to-pink-100 text-violet-800 font-bold px-8 py-3 rounded-full hover:bg-pink-200 transition duration-300">
            Kids Collection
          </Link>
        </div>
      </section>
